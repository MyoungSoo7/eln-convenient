...
const RESULTS = {
  ...
  '2.연구노트': {
    ...
    29: { result: 'FAIL', note: '[BUG] GET /api/tags → 404. 프록시 라우트 미등록 가능성' },
  ...
  '3.전자서명_감사': {
    ...
    16: { result: 'FAIL', note: '[API경로] /api/exports/pdf/:noteId → 400. 경로 불일치 가능성' },
    17: { result: 'SKIP', note: 'BullMQ 비동기 작업' },
    18: { result: 'SKIP', note: 'BullMQ 비동기 작업' },
    19: { result: 'FAIL', note: '[API경로] /api/exports/list → 400. 경로 불일치 가능성' },
    20: { result: 'PASS', note: '알림 목록 200 OK' },
    21: { result: 'PASS', note: '미읽음 카운트 200 OK' },
    22: { result: 'FAIL', note: '[API] PATCH read-all → 400. body 또는 Content-Type 이슈' },
  ...
  '5.예약': {
    ...
    9:  { result: 'FAIL', note: '[API] approve → 400. 승인 시 body 필드 필요 또는 상태 변경됨' },
    10: { result: 'SKIP', note: 'race condition 시뮬레이션 필요' },
    11: { result: 'PASS', note: '반려 200 OK, rejectedReason 기록' },
    12: { result: 'FAIL', note: '[의존성] TC-009 승인 실패로 APPROVED 예약 없음' },
    13: { result: 'FAIL', note: '[의존성] TC-009 승인 실패로 APPROVED 예약 없음' },
    14: { result: 'PASS', note: '종단 상태 전환 → 400 차단' },
    15: { result: 'SKIP', note: '두 번째 researcher 필요' },
    16: { result: 'FAIL', note: '[의존성] TC-009와 동일 — approve API 400' },
  ...
  '7.파일': {
    ...
    9:  { result: 'FAIL', note: '[권한] Researcher로 삭제 → 403. file:delete는 Admin 전용. 테스트 계정 변경 필요' },
