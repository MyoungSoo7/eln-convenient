...
export async function createNote(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    await callAuditLog({
      ...
      console.error('[AUDIT_WARN] note.created audit 기록 실패 (노트 생성은 완료)', {
        noteId: note.id,
...
export async function updateNote(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    await callAuditLog({
      ...
      console.error('[AUDIT_WARN] note.updated audit 기록 실패 (수정은 완료)', {
        noteId: req.params.id,
        err: auditErr instanceof Error ? auditErr.message : auditErr,
      });
    });
...
export async function deleteNote(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    await callAuditLog({
      ...
      console.error('[AUDIT_WARN] note.deleted audit 기록 실패 (삭제는 완료)', {
        noteId: req.params.id,
        err: auditErr instanceof Error ? auditErr.message : auditErr,
      });
    });
...
export async function changeNoteStatus(req: Request, res: Response): Promise<void> {
  try {
    ...
    await callAuditLog({
      ...
      console.error('[AUDIT_WARN] note.status_changed audit 기록 실패 (상태변경은 완료)', {
        noteId: req.params.id,
        err: auditErr instanceof Error ? auditErr.message : auditErr,
      });
    });
...
export async function adminUnlockNote(req: Request, res: Response): Promise<void> {
  try {
    ...
    await callAuditLog({
      ...
      console.error('[AUDIT_WARN] note.admin_unlocked audit 기록 실패 (잠금해제는 완료)', {
        noteId: req.params.id,
        err: auditErr instanceof Error ? auditErr.message : auditErr,
      });
    });
