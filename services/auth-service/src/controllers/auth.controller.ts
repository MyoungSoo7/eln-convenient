...
export async function ssoHook(request: FastifyRequest, reply: FastifyReply) {
  ...
  switch (type) {
    case 'REGISTER': {
      ...
      const existing = await prisma.user.findUnique({ where: { email } });
      if (existing) {
        return { ok: true, message: '이미 등록된 사용자입니다.', userId: existing.id };
      }
      ...
      reply.code(201);
      return { ok: true, message: 'SSO 사용자 등록 완료', userId: newUser.id };
    }
    ...
    case 'LOGIN': {
      ...
      return { ok: true, message: 'LOGIN 이벤트 수신' };
    }
    ...
    case 'UPDATE_PROFILE': {
      ...
      return { ok: true, message: 'UPDATE_PROFILE 이벤트 처리 완료' };
    }
    ...
    case 'DELETE_ACCOUNT': {
      ...
      return { ok: true, message: 'DELETE_ACCOUNT 이벤트 처리 완료' };
    }

    default:
      return { ok: true, message: `이벤트 수신 (미처리): ${type}` };
  }
}
